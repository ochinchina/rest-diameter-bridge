...
- Linux with SCTP support enabled
- Rust and Cargo
...
The SCTP transport is enabled by the Linux-specific dependencies in `Cargo.toml`. The host must also have SCTP kernel support and permission to bind SCTP sockets.

## Start the example

Open four terminals. Run the commands from `examples/sctp`, or use the corresponding paths from the repository root.
...
```bash
../../target/debug/rest-diameter-bridge --config-file sctp-diameter-3.yaml --log-level info
```

```bash
../../target/debug/rest-diameter-bridge --config-file sctp-diameter-2.yaml --log-level info
```

```bash
../../target/debug/rest-diameter-bridge --config-file sctp-diameter-1.yaml --log-level info
```
...
|---|---|---|
| Gateway A | REST ingress and Diameter client | `http://127.0.0.1:8080/diameter`, SCTP `3867` |
| Gateway B | Diameter proxy | SCTP `3868` |
| Gateway C | REST egress and Diameter server | SCTP `3869`, backend `http://127.0.0.1:9090/diameter` |
...
2. Gateway A sends the message to Gateway B over SCTP port `3868`.
...
- Confirm SCTP is available on the host with `cat /proc/net/sctp/assocs` and check the Gateway logs for bind errors.
